...
/*
 ...
 one item per line. Each Elf separates their own inventory from the previous Elf's inventory
...
public class Day01 {
    ...
    public static void main (String [] args) throws FileNotFoundException {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (line.isEmpty()) {
                // Means the current inventory is over. So update the maximum calories carried by an Elf
                if (currentCalories > highestCalories) {
                    highestCalories = currentCalories;
                    highestCaloriesElf = currentElf;
                }
                // Reset the current Elf and their total Calories
                currentElf++;
                currentCalories = 0;
            } else {
                // parsing the line as an integer and add it to the current elf's total calories
                currentCalories += Integer.parseInt(line);
            }
        }

        // Print the result
        System.out.println("Elf " + highestCaloriesElf + " has the most calories: " + highestCalories);
    }
}
